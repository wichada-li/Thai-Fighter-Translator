'use strict';
/**
 * romanize.js — Thai Fighter Name Romanizer (ported from romanize.py)
...
function _load(fname) {
  try { return JSON.parse(fs.readFileSync(path.join(_DIR, fname), 'utf-8')); }
  catch { return {}; }
}

const DB_FIRST = _load('first_names.json');
const DB_LAST  = _load('last_names.json');
const DB_CSV   = _load('csv_word_dict.json');
